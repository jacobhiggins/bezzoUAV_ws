#include "ardrone_simulator/Quadrotor.h"
#include <iostream>
...
*/
  double Ixx = 2.04016e-3,     Ixy = 0,             Ixz = 0;
  double Iyx = 0,              Iyy = 1.56771e-3,    Iyz = 0;
  double Izx = 0,              Izy = 0,             Izz = 3.51779e-3;


  J_ << Eigen::Vector3d(Ixx,Iyx,Izx), Eigen::Vector3d(Ixy,Iyy,Izy), Eigen::Vector3d(Ixz,Iyz,Izz);
  ...
  //arm_length_ = 0.178;
  arm0_x = arm3_x = 0.12586;
  arm1_x = arm2_x = 0.12586;
  arm0_y = arm3_y = 0.12586;
  arm1_y = arm2_y = 0.12586;
  
  coeff_ = Eigen::Matrix<double, 4, 4>::Constant(1.0);
  coeff_(3,1) = coeff_(3, 3) = -1;
  coeff_(1,0) = arm0_x;  coeff_(2,0) = -arm0_y;
  coeff_(1,1) = arm1_x;  coeff_(2,1) = arm1_y;
  coeff_(1,2) = -arm2_x;  coeff_(2,2) = arm2_y;
  coeff_(1,3) = -arm3_x;  coeff_(2,3) = -arm3_y;
...
//  moments(2) = km_*(motor_rpm_sq(0) + motor_rpm_sq(1) - motor_rpm_sq(2) - motor_rpm_sq(3));
  ...
  moments(0) = kf_*(motor_rpm_sq(0) * arm0_x + 
                    motor_rpm_sq(1) * arm1_x -
                    motor_rpm_sq(2) * arm2_x -
                    motor_rpm_sq(3) * arm3_x);

  moments(1) = kf_*(-motor_rpm_sq(0) * arm0_y + 
                     motor_rpm_sq(1) * arm1_y +
                     motor_rpm_sq(2) * arm2_y -
                     motor_rpm_sq(3) * arm3_y);
...
const Eigen::Matrix<double, 4, 4> &Quadrotor::getTorqueArmLength(void) const
{
  return coeff_;
}
